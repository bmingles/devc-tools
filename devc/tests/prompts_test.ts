...
import { confirmLine, confirmReduce, confirmState, runConfirm } from "../tui/prompts.ts";
...
Deno.test("reducer: y/n/enter/esc", () => {
  ...
  assertEquals(confirmReduce(confirmState("Apply?", false), key("enter")).answer, false);
...
Deno.test("runConfirm: 'y' resolves true and echoes the prompt", async () => {
  const out = sink();
  const answer = await runConfirm("Apply?", false, { input: streamOf("y"), output: out.stream });
...
Deno.test("runConfirm: Enter takes the default", async () => {
  const out = sink();
  assertEquals(await runConfirm("Apply?", true, { input: streamOf("\r"), output: out.stream }), true);
  const out2 = sink();
  assertEquals(await runConfirm("Apply?", false, { input: streamOf("\r"), output: out2.stream }), false);
});

Deno.test("runConfirm: Esc resolves false (cancel)", async () => {
  const out = sink();
  assertEquals(await runConfirm("Apply?", true, { input: streamOf("\x1b"), output: out.stream }), false);
});
