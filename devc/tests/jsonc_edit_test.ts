...
function mountsOf(src: string): string[] {
  ...
  return parsed.mounts.map((m) => (typeof m === "string" ? m : JSON.stringify(m)));
}
...
Deno.test("no-fence insert: both fences are appended, user elements untouched", async () => {
  ...
  assertStringIncludes(out, "  // Hand-written mounts, as objects spanning several lines each.\n");
...
Deno.test("empty fence: the preceding element loses its now-trailing comma", async () => {
  ...
  assertEquals(mountsOf(out), ["type=bind,source=/host/keep,target=/container/keep"]);
  assertStringIncludes(out, '    "type=bind,source=/host/keep,target=/container/keep"\n');
...
Deno.test("comma-hostile input round-trips: trailing comma, block comment, ] and // in a string", async () => {
  ...
  const out = writeBlocks(src, "mounts", [
    { id: "projects", lines: ['"type=bind,source=/root/p,target=/workspaces/p"'] },
    { id: "skills", lines: [] },
  ]);
  const parsed = parseJsonc(out) as { mounts: string[]; runArgs: string[]; name: string };
  ...
  assertStringIncludes(out, "    /* a block comment, with a comma and a ] in it */\n");
});
...
Deno.test("missing array: ensureArray inserts the key with the right comma", () => {
  ...
  assertEquals((parseJsonc(a) as { mounts: unknown[]; name: string }).mounts, []);
