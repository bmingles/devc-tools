...
// Resolves `rel` (e.g. git rev-parse's `--show-cdup`/`--git-common-dir` output,
// which may be relative) against `base`, both using "/" separators. `rel` may
// also already be absolute, in which case it's returned as-is.
